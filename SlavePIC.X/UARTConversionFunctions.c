...
extern float speedUART;   //speed variable for PID
extern char directionUART;
extern char distanceUART;

float UART_to_data(char *CommArray, char *dir_dest, char *dist_dest){
    
    float speedUART = (CommArray[0]-'0')*10 + (CommArray[1]-'0') + (CommArray[2]-'0')*0.1 + (CommArray[3]-'0')*0.01 + (CommArray[4]-'0')*0.001;
    char directionUART = CommArray[5];
    char distanceUART = CommArray[6];
    

    float new_speed_data;
